...
public abstract class WizardPreferencesPage extends WizardPage implements
	...
	private String getNoOptionsMessage() {
		return PreferencesMessages.get().WizardPreferencesPage_noOptionsSelected;
	}
...
//		groupData.horizontalIndent = LayoutConstants.getIndent();
		...
		description.setText(PreferencesMessages.get().WizardPreferences_description);
	...
	private void addSelectionButtons(Composite composite) {
		...
		selectAllButton.setText(PreferencesMessages.get().SelectionDialog_selectLabel);
		...
		deselectAllButton.setText(PreferencesMessages.get().SelectionDialog_deselectLabel);
	...
	protected void createDestinationGroup(Composite parent) {
		...
		destinationBrowseButton
				.setText(PreferencesMessages.get().PreferencesExport_browse);
	...
	protected void createOptionsGroup(Composite parent) {
		...
		overwriteExistingFilesCheckbox
				.setText(PreferencesMessages.get().ExportFile_overwriteExisting);
	...
	protected boolean ensureDirectoryExists(File directory) {
		if (!directory.exists()) {
			if (!queryYesNoQuestion(PreferencesMessages.get().PreferencesExport_createTargetDirectory)) {
				return false;
			}
			...
			if (!directory.mkdirs()) {
				MessageDialog
						.open(
								...
								PreferencesMessages.get().PreferencesExport_error,
								PreferencesMessages.get().PreferencesExport_directoryCreationError,
								SWT.SHEET);
				return false;
			}
		}
	...
	protected boolean queryYesNoQuestion(String message) {
		MessageDialog dialog = new MessageDialog(getContainer().getShell(),
				PreferencesMessages.get().Question, (Image) null, message,
	...
	protected boolean ensureTargetIsValid(File file) {
		if (file.exists()) {
			if (!getOverwriteExisting()) {
				String msg = NLS
						.bind(
								PreferencesMessages.get().WizardPreferencesExportPage1_overwrite,
	...
	public String queryOverwrite(String pathString) {
		...
		if (path.getFileExtension() == null || path.segmentCount() < 2) {
			messageString = NLS.bind(
					PreferencesMessages.get().WizardDataTransfer_existsQuestion,
					pathString);
		} else {
			messageString = NLS
					.bind(
							PreferencesMessages.get().WizardDataTransfer_overwriteNameAndPathQuestion,
							path.lastSegment(), path.removeLastSegments(1)
									.toOSString());
		}

		final MessageDialog dialog = new MessageDialog(getContainer().getShell(), PreferencesMessages.get().Question,
				null,
