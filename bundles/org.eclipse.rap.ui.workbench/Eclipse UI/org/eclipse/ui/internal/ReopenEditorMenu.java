...
//        sb.append(" "); //$NON-NLS-1$
	...
	private void addClearHistory(Menu menu, int nItems) {
		...
		miClear.setText(WorkbenchMessages.get().OpenRecentDocumentsClear_text);
		miClear.addSelectionListener(new SelectionAdapter()
		{
			@Override
			public void widgetSelected(SelectionEvent e)
            {
				 Arrays.stream(history.getItems()).forEach(item -> {
						history.remove(item);
					});
            }
		});
