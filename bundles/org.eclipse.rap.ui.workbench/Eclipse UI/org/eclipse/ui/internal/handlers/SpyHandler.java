	...
	protected void showTooltip(Control control) {
		...
		while (control != null) {
			if (control instanceof Table) {
				...
				if (table.getSelectionCount() == 1) {
					...
					contributionInfo = getContributionInfo(tableItem.getData(),
							ContributionInfoMessages.get().ContributionInfo_TableItem);
				...
				if (tree.getSelectionCount() == 1) {
					...
					contributionInfo = getContributionInfo(treeItem.getData(),
							ContributionInfoMessages.get().ContributionInfo_TreeItem);
					if (contributionInfo != null) {
			...
			if (control instanceof Shell)
				optionalElementType = ContributionInfoMessages.get().ContributionInfo_Window;
			else
