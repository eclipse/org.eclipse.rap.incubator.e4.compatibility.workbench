...
public class Throttler {
	...
	/**
	 * Initializes a new throttler object that will throttle the execution of the
	 * given runnable in the {@link Display#getThread() UI thread} of the given
	 * display. The throttler will ensure that the runnable will not run more than
	 * every {@code minWaitTime}, even if it is {@link #throttledExec() executed}
	 * more often.
	 *
	 * @param display     the display owning the thread onto which the runnable will
	 *                    be executed.
	 * @param minWaitTime the minimum duration between each execution of the given
	 *                    runnable.
	 * @param runnable    the runnable to throttle.
	 */
	public Throttler(Display display, Duration minWaitTime, Runnable runnable) {
		...
		if (minWaitTime.toMillis() > Integer.MAX_VALUE) {
			throw new IllegalArgumentException("Minimum wait time must be smaller than " + Integer.MAX_VALUE); //$NON-NLS-1$
		}
	 ...
	 * nothing if it has already been scheduled but not executed yet.
	 */
